...
class WalletViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val rewardEngine = WalletRewardEngine(walletRepository, walletEngine, analyticsEngine)
    ...
    val userIdFlow: Flow<String> = activeUser.map { it?.email ?: "guest" }
    ...
    fun cashOutUPI(
        ...
        viewModelScope.launch {
            ...
            val success = walletEngine.deductCoins(
                userId = userId,
                coins = coinCost,
                activityTitle = "Withdrew Cash to UPI",
                activityDesc = "Requested cashout of ₹$amountRupees to $upiIdOrMobile",
                type = "WITHDRAW"
            )
            ...
            if (success) {
                ...
                val withdrawal = WithdrawalHistoryEntity(
                    withdrawalId = "WITHDRAW_${UUID.randomUUID().toString().take(8)}",
                    userId = userId,
                    upiId = upiIdOrMobile,
    ...
    fun redeemGiftVoucher(
        ...
        viewModelScope.launch {
            ...
            val success = walletEngine.deductCoins(
                ...
                activityDesc = "Purchased ₹$estimatedValueRupees gift card",
                type = "REDEEM"
            )

            if (success) {
                val redeem = RedeemHistoryEntity(
                    redeemId = "REDEEM_${UUID.randomUUID().toString().take(8)}",
                    userId = userId,
                    brandName = brandName,
                    coinsCost = coinsCost,
